...
const OperationTime = db.OperationTime
...
const createCalendarData = async (year: number): Promise<void> => {
    logger.info(year);

    try {
        const weekdayOperationTime = await OperationTime.findOne({
        ...
        const weekendOperationTime = await OperationTime.findOne({
            attributes: ['OperationTimeId'],
            where: {
                name: '토',
            },
        ...
        if (!weekdayOperationTime || !weekendOperationTime || !holidayOperationTime) {
            throw new Error('OperationTime not found for one or more types');
        }

        const weekdayOperationTimeId = weekdayOperationTime.OperationTimeId;
        const weekendOperationTimeId = weekendOperationTime.OperationTimeId;
        const holidayOperationTimeId = holidayOperationTime.OperationTimeId;
        ...
        for (let month = 0; month < 12; month++) {
            ...
            for (let day = 1; day <= lastDayOfMonth.getDate(); day++) {
                ...
                let operationTimeId;
                if (dayType === "토요일") {
                    operationTimeId = weekendOperationTimeId;
                } else if (dayType === "일요일") {
                    operationTimeId = holidayOperationTimeId;
                } else {
                    operationTimeId = weekdayOperationTimeId;
                }
...
const holidayAPI = async (number_year: string): Promise<void> => {
    ...
    if (!holidayOperationTime) {
        throw new Error('OperationTime for 공휴일 not found');
    }

    let string_month: string;
    ...
    for (let i = 1; i < 13; i++) {
        ...
        queryParams += '&' + encodeURIComponent('solYear') + '=' + encodeURIComponent(number_year.toString()); /* */
        if (i < 10) {
