...
//완료 (프론트에서 받는 형식 확인)
router.get('/calendar',isAuthenticatedAdmin,async function(req,res){//이번달 기준 예약 가능 날짜
...
function returnReservationContent(productTire,productWheel,reservationMasterData){
    ...
    productTire.map((product, i)=>{
        ...
        // logger.info(tireLocation)
        // logger.info('tireLocation: '+JSON.stringify(tireLocation))
        tireData[i] = {
...
router.post('/',isAuthenticatedAdmin,async function(req,res){
    ...
    const name = reservationContent.name 
    const number =reservationContent.number
    const userId =  -1;
...
async function returnTodayReservations(reservationTimeDatas){
    ...
    for(let i=0;i<reservationTimeDatas.length; i++){
        logger.info(i)
        //logger.info('reservationTimeDatas[i]'+JSON.stringify(reservationTimeDatas[i]))
        let reReservationContent =[];
        let reservationMasters = await sequelize.query(`select * from ReservationMasters join ReservationTimes on ReservationMasters.ReservationTimeId = ReservationTimes.ReservationTimeId where ReservationMasters.ReservationTimeId = '${reservationTimeDatas[i].ReservationTimeId}' AND ReservationMasters.deletedAt IS NULL`, { type: sequelize.QueryTypes.SELECT })
        logger.info('reservationMasters: '+JSON.stringify(reservationMasters[i]))
        for(let x=0;x<reservationMasters.length;x++){
            //logger.info('reservationMasters[x]'+JSON.stringify(reservationMasters[x]))
            let {reservationTiredata, reservationWheeldata, reservationMasterDataWithUserData} = await retrunProductData(reservationMasters[x]);
...
router.get('/reservedata',async function(req,res){ //하루치 예약 모두
   ...
   let result
    try {
        ...
        let reservationContent = await returnTodayReservations(reservationTimeDatas);
        //let todayReservation = returnTodayReservation(reservationContent)
        result = returnResult(reservationContent,date)
...
router.post('/confirmed',isAuthenticatedAdmin,async function(req,res){ //관리자가 예약 신청한거 확인후 승인 or 취소
    try {
        const data = req.body.data
        const reservationMasterId = data.ReservationMasterId;

        console.log(reservationMasterId)
...
router.post('/isActive',isAuthenticatedAdmin,async function(req,res){
    const data = req.body.data;
    try {
        //해당 reservationTime 에서 예약이 되어있는지 확인
        const reservationTimeData = await ReservationTime.findOne({
            where:{
                ReservationTimeId: data.ReservationTimeId,
            }
        })
        if(data.reservationPossible == false && reservationTimeData.availableNumberOfReservation == reservationTimeData.numberOfReservation){ //예약 비활성화 요청  & 예약이 없는 상황
            //예약이 없으니까 reservationPossible 를 false로 업데이트
            ...
            logger.info(updatedReservationTime)
            // 업데이트가 성공했는지 확인
         if (updatedReservationTime > 0) {
            res.json({
                status:{
                    code:2000,
                    message: "해당 예약시간을 비활성화 시켰습니다."
                },
            ...
            res.json({
                status:{
                    code:4000,
                    message: "비활성화에 실패하였습니다."
                },
                //data:error ?? null
            })
        }
        }else if(data.reservationPossible == false && reservationTimeData.availableNumberOfReservation < reservationTimeData.numberOfReservation){ //예약 비활성화 요청 & 예약 있는 상황
            //예약이 있는상태 -> 해당 예약 취소후 가능
            res.json({
                status:{
                    code:4000,
                    message: "해당시간에 이미 예약이 존재합니다."
                },
                
            })
        }else if(data.reservationPossible == true){//예약 활성화 요청
            const updatedReservationTime = await ReservationTime.update({
        ...
        logger.error('/isActive 오류발생: '+error)
        res.json({
            status:{
                code:4000,
                message: error
